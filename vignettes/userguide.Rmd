...
This function should check if the key already exists in the cache - if it does, the path in the cached file can be used directly, otherwise it should be downloaded from the API.
...
Application developers are strongly advised to use some kind of caching to reduce bandwidth and data egress costs.

# Customizing the authentication 

Developers can globally specify the authentication procedure by through the `identityAvailable()` and `identityHeaders()` functions.
To demonstrate, we'll be using GitHub personal access tokens:
