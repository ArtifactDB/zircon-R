...
#' @param md5.field String specifying the metadata field containing the MD5 checksums.
#' If \code{NULL}, it is assumed that no such metadata field exists.
...
initializeUpload <- function(dir, files, start.url, auto.dedup.md5=FALSE, dedup.md5=NULL, md5.field="md5sum", dedup.link=NULL, expires=NULL, user.agent=NULL, override.key=NULL, api.version=2) {
    ...
    if (length(md5.files) && is.null(md5.field)){ 
        stop("cannot specify 'md5.files' if 'md5.field=NULL'")
    }
...
.format_files <- function(dir, files, auto.dedup.md5, md5.field, api.version) {
    ...
    for (f in which(!is.placeholder)) {
        ...
        if (!endsWith(fname, ".json") && !is.null(md5.field)) {
